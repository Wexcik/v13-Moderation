...
module.exports = { name: "özelkomut", aliases: ["permkurulum", "talent-perm"],  category: "Owner", desc: "---------",
    execute: async (client, message, args, author, channel, guild) => {
        ...
        if(!args[0]) return message.channel.send((`${client.emojis.cache.find(x => x.name === "wex_carpi") || "Emoji Bulunamadı"} \`Hatalı İşlem\` Bir Argüman Belirt \`(ekle,sil,list)\` `))
        if (args[0] === "oluştur" || args[0] === "ekle") {
          ...
          if(!args2) return message.channel.send(`${client.emojis.cache.find(x => x.name === "wex_carpi") || "Emoji Bulunamadı"} \`Hatalı İşlem\` Komutu örnekteki gibi kullanın \`${Settings}permkurulum KomutAdı Verilecek Rol - Yetkili Rol\``).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
          let roller = args2[0].split(" ").map(rol => message.guild.roles.cache.get(rol.replace("<@&", "").replace(">", "")));
          if (!roller) return message.channel.send(`${client.emojis.cache.find(x => x.name === "wex_carpi") || "Emoji Bulunamadı"} \`Hatalı İşlem\` Komutu örnekteki gibi kullanın \`${Settings}permkurulum KomutAdı Verilecek Rol - Yetkili Rol\``).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
          let yetkiliRol = args2[1].split(" ").map(rol => message.guild.roles.cache.get(rol.replace("<@&", "").replace(">", "")));
          if (!yetkiliRol) return message.channel.send(`${client.emojis.cache.find(x => x.name === "wex_carpi") || "Emoji Bulunamadı"} \`Hatalı İşlem\` Komutu örnekteki gibi kullanın \`${Settings}permkurulum KomutAdı Verilecek Rol - Yetkili Rol\``).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
