...
module.exports = { name: "ban-affı", aliases: ["banaffı", "aff"], category: "Owner", desc: "Tüm Banları Kaldırma Komutu",
    execute: async (client, message, args, author, channel, guild) => {
        ...
        if(!message.member.permissions.has("8") && !Server.ownerRoles.some(rol => message.member.roles.cache.has(rol))) return message.reply(AutoRepy.YetersizYetki).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
        ...
        await banneds.forEach(async member => {
          ...
          message.reply("Tüm Banlar Açılıyor."), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
        });
        if (message) return;
      }
    }    