...
module.exports = { name: "erkek", aliases: ["e", "man"], category: "Register", desc: "Kullanıcıyı Erkek Olarak Kayıt Etme Komutu",
    ...
    if(!message.channel.id.includes(Server.registerChannel)) return message.reply("Bu komutu sadece register kanalında kullanabilirsin.").then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    if(Server.manRole && Server.manRole.some(rol => Member.roles.cache.has(rol) && Server.womanRole && Server.manRole.some(rol => Member.roles.cache.has(rol)))) return message.reply(AutoReply.KayıtlıUser).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    Database.findOne({guildID: message.guild.id, victimID: Member.id}, (err, wex) => {
        if(!wex) {
            ...
            message.reply({ embeds: [Embed.setDescription(`${Member} adlı kullanıcı **Erkek** olarak kayıt edildi.`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 20000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
        } else {
