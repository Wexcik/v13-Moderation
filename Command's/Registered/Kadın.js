...
module.exports = { name: "kadın", aliases: ["k", "woman"],  category: "Register", desc: "Kullanıcıyı Kadın Olarak Kayıt Etme Komutu",
    ...
    if(!message.channel.id.includes(Server.registerChannel)) return message.reply("Bu komutu yanlızca register kanalında kullanabilirsin."), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    const Member = message.mentions.members.first() || message.guild.members.cache.get(args[0])
    if(!Member) return message.reply(AutoReply.UyeBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
    if (Server.tagliAlim === true && !Member.user.username.includes(Server.sunucuTag) && !Member.roles.cache.has(Server.specialRole) && !Member.roles.cache.has(Server.boosterRole)) return message.channel.send(AutoReply.TaglıAlım).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    if(Server.manRole && Server.manRole.some(rol => Member.roles.cache.has(rol) && Server.womanRole && Server.manRole.some(rol => Member.roles.cache.has(rol)))) return message.reply(AutoReply.KayıtlıUser).catch(e => { }), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    Database.findOne({guildID: message.guild.id, victimID: Member.id}, (err, wex) => {
        if(!wex) {
            ...
            message.reply({ embeds: [Embed.setDescription(`${Member} adlı kullanıcı **Kadın** olarak kayıt edildi.`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 20000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
        } else {
        ...
        message.reply({ embeds: [Embede.setDescription(`${Member} adlı kullanıcı **Kadın** olarak kayıt edildi fakat kişini geçmiş isimleri görüntülendi.\n\n${History.join("\n")}\n\n${client.emojis.cache.find(x => x.name === "wex_tik") || "Emoji Bulunamadı"} Kullanıcının toplam \`${History.length}\` adet geçmiş isim kaydı görüntülendi.`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 20000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)}})
