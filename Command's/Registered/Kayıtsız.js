...
module.exports = { name: "kayıtsız", aliases: ["unregister", "unreg"],  category: "Register", desc: "Kullanıcıyı Kayıtsıza Atma Komutu",
    ...
    if (Member.roles.highest.position >= message.member.roles.highest.position && !Settings["Bot.Owner"].includes(message.author.id)) return message.reply(AutoReply.YetersizYetki).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    message.reply({ embeds: [Embed.setDescription(`${Member} adlı kullanıcı **Kayıtsız**'a atıldı.`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
    }
}