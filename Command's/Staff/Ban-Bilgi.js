...
module.exports = { name: "banbilgi", aliases: ["bansorgu", "ban-bilgi"],  category: "Staff", desc: "Ban Sebebini Görme Komutu",
    ...
    let id = args[0]
    if(!args[0] || isNaN(args[0])) return message.reply(AutoReply.birIDbelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
