...
execute: async (client, message, args, ClientEmbed, FlatEmbed) => {
    ...
    message.reply({ embeds: [Embed.setDescription(`${Member} adlı kullanıcı **Şüpheli hesap** kategorisinden çıkarıldı`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
    }
}