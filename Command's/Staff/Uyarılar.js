...
module.exports = { name: "ytsicil", aliases: ["uyarılar"],  category: "Staff", desc: "Kullanıcıya Verilen Uyarıları Görme Komutu",
    ...
    Database.find({victimID: member.id}, async (err, res) => {
        if (res.length <= 0) return message.reply(`${member} adlı kullanıcı hakkında ceza-i işlem verisi bulunamadı.`), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
