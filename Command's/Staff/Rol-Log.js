...
module.exports = { name: "rollog", aliases: ["roleslog"],  category: "Staff", desc: "Kullanıcıya Verilen/Alınan Rolleri Görme Komutu",
    ...
    if (!member) return message.reply(AutoReply.UyeBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
       Database.find({victimID: member.id}, async (err, res) => {
        if(res.length < 1) return message.reply(`${client.emojis.cache.find(x => x.name === "wex_carpi")} ${member} kişisinin herhangi bir rollog kaydına ulaşamadım!`)
        ...
        let msg = await message.channel.send({ components: [row], embeds: [Embed.setDescription(`${client.emojis.cache.find(x => x.name === "wex_soru") || "Emoji Bulunamadı"} ${member} adlı kullanıcının rol geçmiş kaydı.\n${mapped.join("\n")}\n\n${client.emojis.cache.find(x => x.name === "wex_carpi") || "Emoji Bulunamadı"} Kullanıcının \`${res.length}\` adet rol kaydı bulundu.`)] })
