...
module.exports = { name: "kes", aliases: ["bağlantıkes", "bkes" ],  category: "Staff", desc: "Kullanıcıyı Ses Kanalından Atma Komutu",
    ...
    if (Settings.voicekickLimit > 0 && voicekickLimit.has(message.author.id) && voicekickLimit.get(message.author.id) == Settings.voicekickLimit) return message.reply(`${AutoReply.voicekickLimit}`).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    if (!User.voice.channel) return message.reply(AutoReply.seskanalındadegilKes).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    message.channel.send({ embeds: [Embed.setDescription(`${User} adlı üyenin ses bağlantısı başarılı bir şekilde kesildi.`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 25000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
