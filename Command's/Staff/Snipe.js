...
module.exports = { name: "snipe", aliases: ["silinmişmesaj" ],  category: "Staff", desc: "Silinen Mesajı Görme Komutus",
    ...
    if(!mesaj) return message.reply(AutoReply.Mesayok).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    message.channel.send({ embeds: [Embed.setDescription(`${mesaj.content}`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 25000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
