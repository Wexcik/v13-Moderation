...
module.exports = { name: "çek", aliases: ["fastpull"],  category: "Staff", desc: "Kullanıcıyı Yanınıza Çekme Komutu",
    ...
    if (!message.member.voice.channel) {
      return message.reply(AutoReply.sesKanalındaOlmanGerek).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    }
  if (!member) {
    return message.reply(AutoReply.BirÜyeBelirtÇek).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
  }
    if (member.user.bot) {
      return message.reply(AutoReply.BotuCekemezsin).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    }
  if (message.member.id === member.id) {
    return message.reply(AutoReply.KendiniÇekemezsin).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)}
    if (message.member.voice.channel === member.voice.channel) {
      return message.reply(AutoReply.AynıKanaldasınız).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    }
  if (!member.voice.channel) {
    return message.reply(AutoReply.kullanıcıSesKanalındaDeğil).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
  }
    member.voice.setChannel(message.member.voice.channel.id)
      message.reply(`${client.emojis.cache.find(x => x.name === "wex_tik") || "Emoji Bulunamadı"} ${member}, adlı üye ${message.author} tarafından <#${member.voice.channel.id}> kanalından <#${message.member.voice.channel.id}> kanalına çekildi.`).then(e => setTimeout(() => e.delete().catch(() => { }), 30000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
      }
          }
