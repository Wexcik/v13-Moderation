...
module.exports = { name: "nerde", aliases: ["sesbilgi", "nerede"],  category: "Staff", desc: "Kullanıcının Hangi Ses Kanalında Oldugunu Görme Komutu",
    ...
    ${client.emojis.cache.find(x => x.name === "wex_member") || "Emoji Bulunamadı"} Kullanıcı: ${member}, Ses kanalı ${member.voice.channel}
    ───────────────
    ${client.emojis.cache.find(x => x.name === "wex_sescik") || "Emoji Bulunamadı"}  ${txt}
    ───────────────
    ${client.emojis.cache.find(x => x.name === "wex_ayar") || "Emoji Bulunamadı"}  Kullanıcının Mikrafonu **${mic}** , Kulaklığı **${hop}**`)]}), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`) 
