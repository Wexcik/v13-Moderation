...
module.exports = { name: "izinligit", aliases: ["igit"],   category: "User", desc: "Belirttiğiniz Kullanıcı İzin verirse Yanına Gitme Komutu",
    ...
    if (!message.member.voice.channel) { return message.reply(AutoRepy.sesKanalındaOlmanGerekİgit).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)}
    if (!member) { return message.reply(AutoRepy.İzinliGitKullanıcıBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)}
    if (message.member.voice.channel === member.voice.channel) { return message.reply(AutoRepy.AynıKanaldasınız).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)}
    if (!member.voice.channel) { return message.reply(AutoRepy.kullanıcıSesKanalındaDeğil).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)}
    if (member.user.bot) { return message.reply(AutoRepy.botunYanınaGidemezsin).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)}
    ...
    let msg = await message.channel.send({ components: [row], embeds: [embed.setDescription(`Hey ${message.author}, aşağıda bulunan butonlara tıklayarak odaya gelip gelmemesini belirtebilirsin.`)] }); message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
    ...
    button.channel.send({embeds: [embed3.setDescription(`${client.emojis.cache.find(x => x.name === "wex_tik") || "Emoji Bulunamadı"} ${member} adlı kullanıcı odaya bağlanma istediğinizi onayladı.`)] })}
    if(button.customId === "reddet") { 
    button.channel.send({embeds: [embed2.setDescription(`${client.emojis.cache.find(x => x.name === "wex_carpi") || "Emoji Bulunamadı"} ${member} adlı kullanıcı odaya bağlanma istediğinizi reddetti.`)] })}})}}