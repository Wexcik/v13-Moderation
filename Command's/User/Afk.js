...
if(message.member.displayName.includes("[AFK]")) return
    if (reason.includes("discord.gg") || reason.includes("@everyone") || reason.includes("@here") || reason.includes(message.mentions.roles.first())) {
        ...
        message.reply({ embeds: [Embed.setDescription(`Afk moduna giriş yaparken afk sebebine rol veya link belirtemezsin`,{ disableMentions: "everyone" })] }).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
        return;};
Database.findOne({memberID: message.author.id}, (err, res)  => {
    ...
    message.reply({ embeds: [Embed.setDescription(`${victim} başarılı bir şekilde afk moduna giriş yaptın. [ \`${reason}\` ]`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
