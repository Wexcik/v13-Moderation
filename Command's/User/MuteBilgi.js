...
module.exports = { name: "mutebilgi", aliases: ["vmutebilgi", "cmutebilgi", "mb"],   category: "User", desc: "VoiceMute & ChatMute cezanızın bitmesine kalan süreyi görüntüleme komutu",
    execute: async (client, message, args, ClientEmbed, FlatEmbed) => { 
        ...
        if (Settings.mbLimit > 0 && mbLimit.has(message.author.id) && mbLimit.get(message.author.id) == Settings.mbLimit) return message.reply(`${AutoReply.mbLimit}`).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
        Database.find({victimID: member.id, activity: true, Type: "VOICE-MUTE"}, async (err,res) => {
          ...
          message.reply({ embeds: [embed.setDescription(`${client.emojis.cache.find(x => x.name === "wex_ses") || "Emoji Bulunamadı"} **Voice-Mute Bilgisi**
          Kullanıcının aktif olan voice mute bilgileri aşağıda belirtilmiştir.\n\n${sesveri.join(", \n") || "Aktif bir Voice-Mute ceza-i işlemin bulunmuyor."}
          **────────────────────**
          ${client.emojis.cache.find(x => x.name === "wex_mesaj") || "Emoji Bulunamadı"} **Chat-Mute Bilgisi**
          Kullanıcının aktif olan chat mute bilgileri aşağıda belirtilmiştir.\n\n${chatveri.join(", \n") || "Aktif bir Chat-Mute ceza-i işlemin bulunmuyor."}`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 15000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
        })
    })
