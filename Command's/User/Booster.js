...
module.exports = { name: "booster", aliases: ["zengin", "rich"],   category: "User", desc: "Booster iseniz Komutla İsim Değiştirme Komutu",
    ...
    if (Settings.boosterLimit > 0 && boosterLimit.has(message.author.id) && boosterLimit.get(message.author.id) == Settings.boosterLimit) return message.reply(`${AutoReply.boosterLimit}`).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    let Embed = new MessageEmbed().setAuthor(message.guild.name, message.guild.iconURL({dynamic: true})).setColor("RANDOM");
    let isim = args.join(' ')
    if(!isim) return message.reply(AutoReply.BoosterİsimBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_carpi")}`)
    ...
    message.channel.send({embeds: [Embed.setDescription(`Tebrikler kullanıcı adını \`${tag} ${isim}\` olarak değiştirdin.`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
    let isiml = `${tag} ${isim}`
