...
module.exports = { name: "roles", aliases: ["rolsüz"],  category: "Co-owner", desc: "Rolsüz Kullanıcılara Kayıtsız Rolü Verme Komutu",
    execute: async (client, message, args, ClientEmbed, FlatEmbed) => { 
      ...
      if (args[0] == "add") {
          ...
          message.reply({ embeds: [Embed.setDescription(`${client.emojis.cache.find(x => x.name === "wex_beyazboncuk")} Sunucuda Rolü Olmayan Kullanıcılara <@&${Server.unregisterRole}> rolü tanılandı. (\`${bg.size}\`)`)] }).catch((err) => console.log(err), message.react(`${client.emojis.cache.find(x => x.name === "wex_onay")}`)).then(e => setTimeout(() => e.delete().catch(() => { }), 20000))
      } else if (!args[0]) {
...
${client.emojis.cache.find(x => x.name === "wex_beyazboncuk")} Unregister Rol: <@&${Server.unregisterRole}>
